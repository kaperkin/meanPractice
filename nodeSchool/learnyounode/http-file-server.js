...
//  generic TCP server, we should use the http module from Node core. Like
//  the net module, http also has a method named http.createServer() but this
//  one creates a server that can talk HTTP.
//
...
//  use-case.
//
//  http.createServer() also returns an instance of your server. You must
//  call server.listen(portNumber) to start listening on a particular port.
//
...
function callback(request, response){
    ...
    readStream.on('error', console.error).on('open', function(){
